...
class SIDProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.active = false; this.loaded = false; this.frames = 0; this.cursor = 0;
    this.traces = Array.from({length:3}, () => new Float32Array(512));
    this.port.onmessage = ({data}) => {
      try {
        ...
        else if (data.type === 'model') this.core.setModel(data.value);
  ...
  sendSnapshot() {
    // Copy a chronological window; retain ring buffers in the audio thread.
    const waves=this.traces.map(t=>{const out=new Float32Array(512);for(let i=0;i<512;i++)out[i]=t[(this.cursor+i)&511];return out;});
  ...
  process(_inputs,outputs) {
    ...
    try {
      for(let i=0;i<channel.length;i++) {
        ...
        for(let v=0;v<3;v++)this.traces[v][this.cursor]=this.core.samples[v];
